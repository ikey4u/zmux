#!/usr/bin/env bash
# Cross-compile zmux release zips for macOS / Linux / Windows.
#
# Linker choice:
...
# Do not pass -C strip=symbols: Cargo.toml keeps line tables for panic
# backtraces (strip = "debuginfo"). Post-link strip is debuginfo-only.
#
# Env: CARGO_HOME_DIR, CARGO_TARGET_DIR, DIST_DIR, ZMUX_LINUX_GLIBC
...
GLIBC="${ZMUX_LINUX_GLIBC:-2.17}"
CARGO_HOME_DIR="${CARGO_HOME_DIR:-/tmp/zmux-cargo}"
CARGO_TARGET_DIR="${CARGO_TARGET_DIR:-/tmp/zmux-target}"
RUSTUP_HOME="${RUSTUP_HOME:-${HOME}/.rustup}"

VERSION="$(cargo metadata --no-deps --format-version 1 2>/dev/null \
  | sed -n 's/.*"name":"zmux","version":"\([^"]*\)".*/\1/p' | head -n1)"
if [[ -z "${VERSION}" ]]; then
...
# Debuginfo only: keep the symbol table so panic backtraces still resolve.
strip_bin() {
  local triple="$1"
  local bin="$2"
  local tos
  tos="$(os_of_triple "${triple}")"
  echo "==> strip-debug ${tos} $(basename "$bin")"
  case "${tos}" in
    macos)
      if command -v strip >/dev/null 2>&1 && strip -S "$bin" 2>/dev/null; then
        return 0
      fi
      echo "warn: macOS strip -S skipped for $bin" >&2
      ;;
    linux)
      local objcopy
      ...
      echo "warn: no rust-objcopy/llvm-strip; relying on Cargo strip=debuginfo for $bin" >&2
      ;;
    windows)
      local objcopy
      objcopy="$(find_rust_objcopy)"
      if [[ -n "${objcopy}" ]]; then
        "$objcopy" --strip-debug "$bin" || echo "warn: rust-objcopy strip-debug failed for $bin" >&2
        return 0
      fi
      echo "warn: no Windows strip tool; relying on Cargo strip=debuginfo for $bin" >&2
      ;;
  esac
}
...
if [[ "${HOST_KIND}" == "macos" ]]; then
  ...
  if command -v lipo >/dev/null 2>&1 \
    ...
    univ_bin="${DIST_DIR}/.zmux-universal"
    lipo -create \
      -output "${univ_bin}" \
